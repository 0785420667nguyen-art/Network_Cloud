...
\subsection{Tổng quan Cloud Computing}
...
% - Quản lý tập trung bằng Controller
% - Lập trình mạng qua API

\subsubsection{Kiến trúc SDN}
% - Application Layer: Network Applications, Business Applications
% - Control Layer: SDN Controller (NOX, Ryu, OpenDaylight)
% - Infrastructure Layer: Physical switches, Virtual switches

\subsubsection{Mặt phẳng trong SDN}
% - Control Plane: 
%   + Xử lý logic, quyết định định tuyến
%   + Chạy trên Controller
% - Data Plane:
%   + Forward packet theo Flow Table
%   + Chạy trên Switch

\subsubsection{OpenFlow Protocol}
% - Giao thức giao tiếp giữa Controller và Switch
% - Chuẩn của Open Networking Foundation (ONF)
% - OpenFlow Controller, OpenFlow Switch, OpenFlow Channel
% - Flow Table, Flow Entry (Match Fields, Priority, Actions)
% - Packet In và Packet Out

\subsubsection{SDN Controller}
% - Ryu: Python-based, dễ học, phù hợp nghiên cứu
% - OpenDaylight: Java-based, đầy đủ tính năng
% - Vai trò: Quản lý topology, tính toán đường đi, cài đặt Flow Rule
% - Cung cấp Northbound API cho ứng dụng

\subsubsection{Ưu điểm của SDN}
% - Quản lý tập trung
% - Tự động hóa
% - Linh hoạt, programmability
% - Tiết kiệm chi phí, vendor-independent
...
\subsubsection{VPN Site-to-Site}
% - Khái niệm: Kết nối an toàn giữa 2 site qua Internet công cộng
% - Mã hóa traffic end-to-end
% - Các công nghệ: IPSec, OpenVPN, WireGuard
% - Use case: Kết nối chi nhánh với trụ sở chính

\subsubsection{GRE Tunnel}
% - Generic Routing Encapsulation (RFC 2784, 2890)
% - Đóng gói packet Layer 3 vào bên trong packet IP khác
% - Không cung cấp mã hóa (cần kết hợp IPSec nếu cần bảo mật)
% - Hỗ trợ multicast và routing protocol
% - Dùng để kết nối các site trong mô hình SDN

\subsubsection{Nguyên lý hoạt động}
% - Encapsulation: Đóng gói packet gốc
% - GRE Header: Protocol type, flags, checksum
% - Tunneling: Truyền qua mạng IP
% - Decapsulation: Mở gói tại đích
% - Sơ đồ minh họa: Original IP Packet -> GRE Header + Outer IP Header -> Destination

\subsubsection{Ứng dụng trong SDN}
% - Kết nối các OpenFlow switch ở các site khác nhau
% - Tạo overlay network
% - Tunnel traffic giữa các chi nhánh qua Controller trung tâm
