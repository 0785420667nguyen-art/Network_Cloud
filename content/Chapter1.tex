...
\end{itemize}
\paragraph{Mô hình SDN, SD-WAN}
Trong mô hình kết hợp SDN + SD-WAN, SDN tập trung vào việc quản lý và điều khiển mạng LAN hoặc mạng Data Center, trong khi SD-WAN đảm nhiệm kết nối mạng WAN giữa trụ sở, các chi nhánh và Cloud. Hai công nghệ kết hợp với nhau giúp doanh nghiệp xây dựng một hệ thống mạng có khả năng quản lý tập trung, tăng tính linh hoạt, cải thiện khả năng bảo mật, tối ưu hiệu năng truy cập ứng dụng Cloud và dễ dàng mở rộng khi doanh nghiệp phát triển.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/SDN,SD_WAN.png}
    \caption{Sơ đồ mô hình mạng phân tầng 3 lớp}
    \label{fig:SDN,SD_WAN}
\end{figure}
\textbf{Đặc điểm:}
\begin{itemize}
    \item\textbf{SDN/SD-WAN Controller :}đóng vai trò quản lý và điều khiển tập trung các thiết bị mạng,quản trị viên có thể triển khai chính sách mạng từ hệ thống quản lý trung tâm.
    \item\textbf{SDN Switch :} chịu trách nhiệm kết nối và phân đoạn mạng nội bộ.
    \item\textbf{SD-WAN Edge :}chịu trách nhiệm kết nối mạng LAN với mạng WAN
    \item\textbf{SD-WAN Overlay :} cung cấp kết nối logic giữa các Site và Cloud.
\end{itemize}
\textbf{Ưu điểm}

Mô hình kết hợp SDN và SD-WAN mang lại nhiều ưu điểm trong việc quản lý mạng doanh nghiệp, đặc biệt đối với hệ thống có trụ sở chính, các chi nhánh và hạ tầng Private Cloud. Các ưu điểm chính của mô hình bao gồm:

\begin{itemize}
    \item \textbf{Quản lý tập trung:} 
    SDN/SD-WAN Controller cho phép quản trị viên quản lý tập trung các thiết bị mạng và chính sách kết nối. Việc cấu hình và triển khai chính sách có thể được thực hiện từ một hệ thống quản lý tập trung, giúp giảm cấu hình thủ công trên từng thiết bị và đơn giản hóa công tác quản trị mạng.

    \item \textbf{Tối ưu hiệu năng truy cập Cloud:} 
    SD-WAN có khả năng giám sát chất lượng các đường truyền WAN dựa trên các tiêu chí như độ trễ, jitter, tỷ lệ mất gói và băng thông. Dựa trên các thông tin này, hệ thống có thể lựa chọn đường truyền phù hợp với từng loại ứng dụng, từ đó nâng cao hiệu năng truy cập các dịch vụ và ứng dụng trên Private Cloud.

    \item \textbf{Tăng cường bảo mật:} 
    Mô hình có thể kết hợp nhiều cơ chế bảo mật như tunnel SD-WAN được mã hóa, phân đoạn mạng bằng VLAN, chính sách kiểm soát truy cập và Firewall tại khu vực Private Cloud. Các cơ chế này giúp hạn chế truy cập trái phép và tăng cường khả năng bảo vệ dữ liệu doanh nghiệp.

    \item \textbf{Tăng tính sẵn sàng của hệ thống:} 
    SD-WAN hỗ trợ sử dụng đồng thời nhiều đường truyền WAN như Internet, MPLS hoặc 4G/5G. Khi một đường truyền gặp sự cố hoặc chất lượng suy giảm, hệ thống có thể chuyển lưu lượng sang đường truyền khác theo chính sách được cấu hình, góp phần duy trì khả năng kết nối đến các ứng dụng Cloud.

    \item \textbf{Khả năng mở rộng tốt:} 
    Mô hình có thể dễ dàng mở rộng khi doanh nghiệp phát triển và bổ sung thêm các chi nhánh mới. Các SD-WAN Edge mới có thể được tích hợp vào hệ thống và áp dụng các chính sách mạng thông qua cơ chế quản lý tập trung.

    \item \textbf{Phân tách rõ chức năng mạng:} 
    SDN tập trung vào việc quản lý và điều khiển mạng LAN hoặc hạ tầng Data Center, trong khi SD-WAN đảm nhiệm kết nối WAN giữa trụ sở chính, các chi nhánh và môi trường Cloud. Sự phân tách này giúp kiến trúc mạng rõ ràng và thuận lợi cho việc quản lý.
\end{itemize}


\textbf{Nhược điểm}

Bên cạnh những ưu điểm, mô hình SDN kết hợp SD-WAN vẫn tồn tại một số hạn chế cần xem xét trong quá trình triển khai và vận hành:

\begin{itemize}
    \item \textbf{Kiến trúc phức tạp:} 
    Mô hình bao gồm nhiều thành phần như SDN Controller, SD-WAN Controller, SDN Switch, SD-WAN Edge, SD-WAN Cloud Gateway, Firewall và Private Cloud. Do đó, việc thiết kế, cấu hình, giám sát và xử lý sự cố có thể phức tạp hơn so với mô hình mạng truyền thống.

    \item \textbf{Chi phí đầu tư và vận hành cao:} 
    Việc triển khai mô hình yêu cầu đầu tư vào các thiết bị mạng, hệ thống Controller, Firewall, hạ tầng Private Cloud và các đường truyền WAN. Ngoài ra, doanh nghiệp cần đội ngũ nhân sự có kiến thức chuyên môn về SDN, SD-WAN, Cloud và an toàn thông tin.

    \item \textbf{Phụ thuộc vào hệ thống Controller:} 
    Controller đóng vai trò quan trọng trong việc quản lý và triển khai chính sách mạng. Khi Controller gặp sự cố, khả năng quản lý tập trung, giám sát và cập nhật chính sách có thể bị ảnh hưởng. Vì vậy, hệ thống cần được triển khai theo mô hình dự phòng hoặc High Availability (HA) để giảm thiểu rủi ro.

    \item \textbf{Phụ thuộc vào chất lượng đường truyền WAN:} 
    Mặc dù SD-WAN có khả năng lựa chọn đường truyền phù hợp, hiệu năng kết nối vẫn phụ thuộc vào chất lượng của mạng Underlay và nhà cung cấp dịch vụ Internet. SD-WAN không thể kiểm soát hoàn toàn các sự cố hoặc tình trạng tắc nghẽn xảy ra trên mạng của nhà cung cấp dịch vụ.

    \item \textbf{Nguy cơ xuất hiện điểm lỗi đơn:} 
    Nếu hệ thống chỉ triển khai một SD-WAN Cloud Gateway hoặc một Firewall duy nhất, thành phần này có thể trở thành Single Point of Failure (SPOF). Khi xảy ra sự cố, nhiều chi nhánh có thể mất khả năng truy cập vào Private Cloud. Do đó, cần triển khai các thành phần quan trọng theo mô hình dự phòng và High Availability.

    \item \textbf{Phụ thuộc vào nhà cung cấp:} 
    Trong trường hợp sử dụng giải pháp SDN hoặc SD-WAN thương mại, doanh nghiệp có thể phụ thuộc vào hệ sinh thái, phần mềm, bản quyền và API của nhà cung cấp. Việc chuyển đổi sang một nền tảng khác có thể phát sinh thêm chi phí và yêu cầu thay đổi cấu hình hệ thống.
\end{itemize}
